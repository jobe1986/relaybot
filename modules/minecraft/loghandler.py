...
class LogHandler:
	def __init__(self, loop, config, module, prot):
		...
		self.msgre = {
			'Server thread': {
				...
				'ACTION': [re.compile('^(?:\\[Not Secure\\] )?(?P<raw>\\* (?P<name>.+?) (?P<message>.*?))$')],
				'ADVANCEMENT': [
				...
				'DEATH': [
					...
					re.compile('^(?P<message>(?P<name>[^ ]*?) was squashed by a falling anvil while fighting .*?)$'),
					...
					re.compile('^(?P<message>(?P<name>[^ ]*?) walked into a cactus while trying to escape .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was squished too much)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was squashed by .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was roasted in dragon\'s breath)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was roasted in dragon\'s breath by .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) drowned)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) drowned while trying to escape .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) died from dehydration)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) died from dehydration while trying to escape .*?)$'),
					...
					re.compile('^(?P<message>(?P<name>[^ ]*?) hit the ground too hard while trying to escape .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was squashed by a falling block)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was squashed by a falling block while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was skewered by a falling stalactite)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was skewered by a falling stalactite while fighting .*?)$'),
					...
					re.compile('^(?P<message>(?P<name>[^ ]*?) went off with a bang while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) experienced kinetic energy)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) experienced kinetic energy while trying to escape .*?)$'),
					...
					re.compile('^(?P<message>(?P<name>[^ ]*?) was killed)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was killed while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) discovered the floor was lava)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) walked into the danger zone due to .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) went up in flames)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) walked into fire while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) suffocated in a wall)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) suffocated in a wall while fighting .*?)$'),
					...
					re.compile('^(?P<message>(?P<name>[^ ]*?) was burned to a crisp while fighting .*? wielding .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was burned to a crisp while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) fell out of the world)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) didn\'t want to live in the same world as .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) left the confines of this world)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) left the confines of this world while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was obliterated by a sonically-charged shriek)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was obliterated by a sonically-charged shriek while trying to escape .*? wielding .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was obliterated by a sonically-charged shriek while trying to escape .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was speared by .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was speared by .*? using .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was impaled on a stalagmite)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was impaled on a stalagmite while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) starved to death)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) starved to death while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was stung to death)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was stung to death by .*? using .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was stung to death by .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) died because not just the floor is lava)$'),
					re.compile('^(?P<message>.*? showed (?P<name>[^ ]*?) that not just the floor is lava)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was poked to death by a sweet berry bush)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was poked to death by a sweet berry bush while trying to escape .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was killed while trying to hurt .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was killed by .*? while trying to hurt .*?)$'),
					...
					re.compile('^(?P<message>(?P<name>[^ ]*?) withered away while fighting .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was shot by a skull from .*?)$'),
					re.compile('^(?P<message>(?P<name>[^ ]*?) was shot by a skull from .*? using .*?)$'),
	...
	def handle_event(self, loop, module, sender, protocol, event, data):
		...
		if sender == self.config['name']:
			self.log.info('Handling event ' + str(event))
